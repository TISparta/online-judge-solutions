...
int main () {
  ...
  string line;
  while (getline(cin, line) and sz(line)) {
    ...
    for (int i = 0; i < sz(sa.lcp); i++) {
      ...
      ans[sa.lcp[i]] = max(ans[sa.lcp[i]], to - from + 2);
    }
    for (int i = sz(ans) - 2; i >= 0; i--) ans[i] = max(ans[i], ans[i + 1]);
    for (int i = 1; i < sz(ans) and ans[i]; i++) {
      cout << ans[i] << '\n';
    }
    cout << '\n';
  }
